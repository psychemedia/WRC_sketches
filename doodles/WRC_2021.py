...
# # WRC Scraper  2021
#
# The `WRC2021` package provides a means of scraping timing data from the WRC website and working with it in-memory, or storing it in a database.
#
...
class WRCSeasonCategories(WRCSeasonBase):
    ...
    def fetchSeasonCategories(self):
        ...
        self.championship_codes = getSeasonCategories()
...
# We use the `.fetchData()` method so as to try not to be greedy. This way, we can define a class and start to work towards only grabbling the data if we need it.
...
# # zz.fetchStartList()
# # What startlist should we return by default?
...
class WRCRallyStage(WRCRallyStages):
    ...
    def __init__(self, sdbRallyId=None, stageId=None, live=False,
        ...
        if autoseed:
            pass
            #self.fetchData(self.sdbRallyId, stageId)
...
# + tags=["active-ipynb"]
# zz = WRCOverall(stageId=1701, dbname=TESTDB)
# #zz.fetchOverall()
# #zz.rallyId

# + tags=["active-ipynb"]
# zz = WRCOverall(stageId=1701, autoseed=True, dbname=TESTDB)
...
# + tags=["active-ipynb"]
# !rm wrc2021bigtest1.db
# zz = WRCEvent(dbname='wrc2021bigtest1.db')
#
# zz.rallyslurper()
#
# zz.dbtables()
...
# + tags=["active-ipynb"]
# # This seems to clobber the sql export?
# zz.pickle('wrc-montecarlo2021.pickle')

# + tags=["active-ipynb"]
# with open('WRC2021db.sql', 'w') as f:
#     f.write(''.join(zz.dbtables()['sql'].to_list()))
# !head WRC2021db.sql 

# + tags=["active-ipynb"]
# zz.db_connect(dbname='wrc2021bigtest1.db')
# zz.legs
...
class WRC2021(WRCActiveSeasonEvents, WRCEvent):
    """Class for WRC data scrape using 2021 API."""
...
# + tags=["active-ipynb"]
# rallies, eventdays, zz3 = WRC2021().getActiveSeasonEvents()
#

# + tags=["active-ipynb"]
# #zz = WRCEvent(slurp=True)
# # Next thing to do is probably to specify a selected rally or set the active rally as the current rally

# + tags=["active-ipynb"]
# zz = WRCEvent(autoseed=True, sdbRallyId=1695)
# zz.getPenalties()
